...
const  apiKey = '3b712ebc109bc87b541a0abaa0f64b85';
const unit = 'metric';
//request from external Server to our server 'API'
const url = `https://api.openweathermap.org/data/2.5/weather?q=${query}&appid=${apiKey}&units=${unit}`;
...
https.get(url, function(response){
  ...
  response.on('data', function(data){
    ...
    //response from our server to client server
    response.write(`<p>The weather description is ${weatherDescription}</p>`)
    response.write(`<h1>The Tempereture in ${query} is ${temp} in Celcuis Degrees. </h1>`)
