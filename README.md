...
-	Paragraph tag <p></p>.
-	emphasis  Tag <em> </em> tells the browser that the words between it is emphasis, not just about style.
-	italic tag <i></i> style element.
-	Stong tag <strong></strong> tells the browser that the words between it is Strong Importance Element.
-	Bold tag <bold></bold> styles bold.

###2.5. HTML Lists
-	Ordered list <ol></ol>.
-	There is two types of lists.
-	Ordered lists <ol></ol> and unordered lists <ul></ul>.
-	Each type has children <li></li> for each item of the list.
...
###3.1. HTML Tables
-	Tables main tag <table></table> and everything between is the actual content.
-	To make row use tag <tr></tr>.
-	To make cell inside the row <td></td>.
...
###3.2. HTML forms
-	main tag <form></form> to define what should go into our form.
-	Label tag <label></label> to some text
-	To make input change the type to what we need.
-	Textarea to write message <textarea></textarea>.
...
-	<hr> has border style default values.
